using DBTBalanceRevivedLSSJ4Fix.Model;
using DBZGoatLib;
...
namespace DBTBalanceRevivedLSSJ4Fix.Buffs;
...
public class LSSJ4Buff : Transformation
{
    ...
    public override SoundData SoundData() => new("DBZMODPORT/Sounds/SSJAscension", "DBZMODPORT/Sounds/SSJ3", 260);
    public override AuraData AuraData() => new("DBTBalanceRevivedLSSJ4Fix/Assets/LSSJ4Aura", 4, BlendState.AlphaBlend, new Color(250, 74, 67));
    public override Gradient KiBarGradient() => new Gradient(new Color(255, 56, 99)).AddStop(1f, new Color(156, 0, 34));

    public override string HairTexturePath() => "DBTBalanceRevivedLSSJ4Fix/Assets/LSSJ4Hair";
...
public class LSSJ4Panel : TransformationTree
{
    ...
    [
        new Node(4,3,"LSSJ4Buff","DBTBalanceRevivedLSSJ4Fix/Buffs/LSSJ4Buff","Only defeating a foe of cosmic proportions can unlock this power.",UnlockCondition,DiscoverCondition)
    ];

    public static bool UnlockCondition(Player player)
    {
        return player.GetModPlayer<BPlayer>().LSSJ4Achieved && player.GetModPlayer<GPlayer>().Trait == "Legendary";
    }
    public static bool DiscoverCondition(Player player)
    {
