...
namespace DBTBalanceRevivedLSSJ4Fix.Helpers;
...
internal sealed class BuffHooks
{
    ...
    {
        var kaioken = TransformationHelper.Kaioken;
        var superKaioken = TransformationHelper.SuperKaioken;

        var currentDisplayString = string.Empty;
        if (self.Type == kaioken.getBuffId || self.Type == superKaioken.getBuffId)
        {
            currentDisplayString = self.kaiokenLevel switch
            {
                2 => "(x3)\n",
                3 => "(x4)\n",
                4 => "(x10)\n",
        ...
        var dmg = self.damageMulti - 1f;
        var speed = self.speedMulti - 1f;
        var DrainRate = 60f * self.kiDrainRate;
        var DrainRateMastered = 60f * self.kiDrainRateWithMastery;
        var UsageRate = self.attackDrainMulti - 1f;
        var defense = self.baseDefenceBonus;
        ...
        if (self.healthDrainRate != 0)
            sb.AppendLine($"Life Drain: -{self.healthDrainRate / 2:N0}/s.");
